import { CommitCategory, ParsedCommit, MutableGroupedCommits, GroupedCommits } from '../types';
 ...
 */
export function parseCommitMessage(hash: string, message: string): ParsedCommit {
  const match = CONVENTIONAL_RE.exec(message);

  if (match) {
    const [, prefix, scope, body] = match;
    const category = PREFIX_MAP[prefix.toLowerCase()] ?? CommitCategory.Other;
    return {
      hash,
      message: body.trim(),
      category,
